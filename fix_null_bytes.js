...
// Read file as buffer and strip null bytes that might have corrupted the string representation
...
const endMarker = 'function showTT(e, z, r) {';
...
const functionsToInject = `const RESALE_FULL_DATA_KR = ${tbData};
    ...
    function renderZone(id) {
      ...
      var rowKeys = Object.keys(zd.rows).map(function(k) { return parseInt(k, 10) || k; }).sort(function(a,b) {
         if (typeof a === 'number' && typeof b === 'number') return a - b;
         return String(a).localeCompare(String(b));
      });

      rowKeys.forEach(function (rStr) {
        var r = String(rStr);
        if (zd.rows[r].length > 0) {
          var firstSeat = zd.rows[r][0];
          var text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
          text.setAttribute('x', b.x - 15);
...
console.log("Successfully rebuilt index.html with clean utf8 strings and correct functions!");
