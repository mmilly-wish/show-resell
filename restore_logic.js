...
const marker = "      ttEl.style.left = x + 'px'; ttEl.style.top = y + 'px';\n    }\n    function hideTT() { ttEl.classList.remove('on'); }";
...
const replacement = `
    ...
    function updateSeatVisuals() {
      ...
      document.querySelectorAll('.seat-rect').forEach(function (el) {
        ...
        if (c.kr > 0 || c.cn > 0) { var count = c.kr + c.cn; var alpha = Math.min(0.95, 0.30 + (count * 0.05)); el.setAttribute('fill', 'rgba(244, 67, 54, ' + alpha + ')'); el.setAttribute('fill-opacity', '1'); }
        else {
...
if (html.indexOf('function updateSeatPremium()') === -1) {
  let newHtml = html.replace(marker, replacement);
  fs.writeFileSync('index.html', newHtml, 'utf8');
  console.log('Restored functions successfully.');
} else {
